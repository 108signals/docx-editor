...
/** Resolve the image (or shape) node at `pmPos`, or null if it isn't one. */
function imageNodeAt(view: EditorView, pmPos: number): PMNode | null {
  const node = view.state.doc.nodeAt(pmPos);
  // Shapes share the image resize/drag commit path — same width/height/position
  // attrs, interacted with identically (#656 shape support).
